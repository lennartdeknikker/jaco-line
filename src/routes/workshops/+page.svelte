			...
			{#each data.workshops as workshop}
				<article class="workshop-card">
					...
					<div class="workshop-content">
						...
						{/if}
						<Button variant="primary" on:click={() => openForm(workshop)}>Inschrijven</Button>
					</div>
				</article>
			{/each}
		</div>
...
{#if showForm && selectedWorkshop}
	<div
		class="modal-overlay"
		on:click={closeForm}
		on:keydown={(e) => (e.key === 'Escape' || e.key === 'Enter') && closeForm()}
		role="button"
		tabindex="0"
		aria-label="Sluiten"
	>
		<div class="modal-content" on:click|stopPropagation role="dialog" aria-labelledby="modal-title">
			<button class="modal-close" on:click={closeForm} aria-label="Sluiten">×</button>
			...
			{#if submitted}
				<div class="success-message">
					...
					<Button variant="primary" on:click={closeForm}>Sluiten</Button>
				</div>
			{:else}
				<form on:submit={handleSubmit} class="workshop-form">
					{#if error}
